...
class otezla_hover_data_freequency_details(BaseModel):
    Refresh_Freequency: str = Field(
        alias="Refresh Frequency", Example="Weekly (last refreshed  02/10/2023)"
    )
    Data_Source: str = Field(alias="Data Source", Example="CD&A")
...
class otezla_hover_data_nbrx_volume_by_specialties(BaseModel):
    c4w_vs_p4w: otezla_hover_data_freequency_details
    volume_by_specialties: otezla_hover_data_freequency_details
    r4w_goal: otezla_hover_data_freequency_details


class otezla_hover_data_us_leading(BaseModel):
    copay_penetration: Optional[otezla_hover_data_freequency_details] = Field(
        alias="Copay Penetration"
    )
    nbrx_fulfillment: Optional[otezla_hover_data_freequency_details] = Field(
        alias="NBRx Fulfillment"
    )
    pso_bio_naive_share: otezla_hover_data_freequency_details = Field(
        alias="Copay Penetration"
    )


class otezla_hover_data_worldwide_lrs(BaseModel):
    Last_Refreshed: str = Field(alias="Last Refreshed", Example="02/28/2023")
    Data_Source: str = Field(alias="Data Source", Example="EDW")


class otezla_hover_data_us_execution(BaseModel):
    share_of_voice: otezla_hover_data_freequency_details
    call_attainment: otezla_hover_data_freequency_details


class otezla_hover_data_global_brand_sales_drivers(BaseModel):
    volume_market_mktshare_us: otezla_hover_data_freequency_details
    sales: otezla_hover_data_freequency_details
    volume_market_mktshare_o_us: otezla_hover_data_freequency_details


class otezla_hover_data_us_lagging(BaseModel):
    TRX: otezla_hover_data_freequency_details
    demand_components_of_growth: otezla_hover_data_freequency_details
    net_sales: otezla_hover_data_freequency_details


class otezla_hover_data_details(BaseModel):
    us_leading: otezla_hover_data_us_leading
    worldwide_lrs: otezla_hover_data_worldwide_lrs
    us_execution: otezla_hover_data_us_execution
    global_brand_sales_drivers: otezla_hover_data_global_brand_sales_drivers
    us_lagging: otezla_hover_data_us_lagging


class otezla_hover_data(BaseModel):
    status: Optional[str]
    message: Optional[str]
    data: Optional[otezla_hover_data_details]
